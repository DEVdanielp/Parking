  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.form}>
        ...
        <button onClick={handleAddSpot}>Agregar</button>
      </div>

      <h3>Listado</h3>
      {loading ? 'Cargando...' : (
        <table className={styles.table}>
          <thead><tr><th>Id</th><th>Code</th><th>Type</th><th>Activo</th><th>Acciones</th></tr></thead>
          <tbody>
            {list.map(s => (
              <tr key={s.ID}>
                <td>{s.ID}</td>
                  ...
                  <button onClick={async () => 
                    {
                      try{
                        if(await spotsSvc.UpdateSlotStatus(s.ID!, s.Activa, s.Title)){
                          alert(`Se ha ${s.Activa === "Activa" ? "desactivado" : "activado"} con éxito la celda`);
                          load()
                        }
                      }catch(error){
